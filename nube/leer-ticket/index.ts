...
function systemPromptFoto(): string {
  return [
    ...
    "\"nota\": si el ticket tiene VARIOS artículos distintos (ej. supermercado), listalos separados por coma para",
    "poder identificar la compra de un vistazo (ej. \"Fideos, Aceite, Yerba, Pan\") — si son muchos, listá los 4 o 5",
    "más caros/importantes y agregá \"y otros\" al final. Si en cambio es la compra de UN solo producto o servicio",
    "(ej. combustible, un service, una entrada), poné el nombre de ese producto/servicio y, SI el ticket muestra el",
    "IVA discriminado (una línea tipo \"IVA 21%\" o \"IMPORTE TOTAL IVA\" con un monto), agregalo entre paréntesis al",
    "final, ej. \"Nafta Súper (IVA 21%: $ 2.898)\" — solo si ese dato realmente se lee, nunca lo calcules ni lo",
    "inventes. Si no se distinguen los artículos/producto con claridad, poné el nombre del comercio (ej. \"Super Coto\").",
...
function systemPromptPagina(texto: string): string {
  return [
    ...
    "\"nota\": si hay VARIOS artículos, listalos separados por coma (los 4-5 más importantes si hay muchos,",
    "agregando \"y otros\"). Si es UN solo producto/servicio, su nombre y, si el texto muestra el IVA discriminado",
    "con un monto, agregalo entre paréntesis (ej. \"Nafta Súper (IVA 21%: $ 2.898)\") — solo si ese dato está",
    "realmente en el texto. Si no se distingue nada de esto, el nombre del comercio.",
